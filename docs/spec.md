...
| GET    | `/api/admin/users`      | 목록. `q`(이메일/이름 검색), `role`(`admin`\|`user`), `is_active`(`true`\|`false`), `page`(기본 1), `size`(기본 20, 최대 100) → `{items, total}`. 보내지 않은 조건은 거르지 않는다. 최근 가입자(`created_at`)부터, 같으면 id가 큰 쪽부터 |
...
| `/admin/users`     | 사용자 표(No., 이름, 이메일, 역할, 상태, 마지막 로그인, 가입일), 최근 가입자가 맨 위. 검색, 역할 필터(모든 역할/관리자/사용자), 상태 필터(모든 상태/활성/비활성), 이전/다음 페이지. 필터는 고르면 바로 적용되고, 검색어나 필터를 바꾸면 1페이지로 간다. No.는 전체 개수부터 거꾸로 세어 페이지를 넘어 이어진다(총 25명이면 1페이지 첫 줄은 25, 2페이지 첫 줄은 5, 가장 오래된 가입자가 1). 마지막 로그인이 없으면 `-`. 검색어, 필터, 페이지는 URL 쿼리(`?q=&role=&status=active\|inactive&page=`)에 유지하고, 알 수 없는 필터 값은 무시한다. 페이지가 전체 페이지 수를 넘으면(마지막 페이지의 사용자를 모두 삭제한 경우 등) 마지막 페이지로 옮긴다. 팝업에서 추가, 저장, 삭제한 뒤 목록 위에 뜨는 안내 문구는 10초 동안 보이고 사라진다 |
...
| (팝업) 사용자 수정 | 목록에서 **이름**을 누르면 목록 화면 위에 연다. 별도 경로는 없다. 이메일은 읽기 전용, 이름, 역할, 활성 여부, 새 비밀번호. 바꾼 항목만 저장. 삭제 버튼은 확인 후 삭제. 저장이나 삭제에 성공하면 팝업을 닫고 안내 문구와 함께 목록을 다시 불러온다. 자기 자신이면 역할, 활성 여부, 삭제를 막는다. 끌어 옮기기와 닫기는 추가 팝업과 같다 |
...
7. (완료) 사용자 수정/삭제를 팝업으로 변경 (`/admin/users/:id` 경로 삭제)
8. (완료) 사용자 목록에 역할/상태 필터 추가, 최근 가입자부터 정렬
